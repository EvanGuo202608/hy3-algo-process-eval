...
demo 默认展示 `A01-pack-cost` 中“代码通过测试，但公开推理过程使用了错误的整数除法依据”的样本；也可以切换到 `A02-interval-removal` 展示“端点处理解释错误但代码正确”的样本，切换到 `B01-pairing` 展示“贪心证明错误但代码正确”的样本，切换到 `B02-cut-height` 展示“二分单调性解释反向但代码正确”的样本，或切换到 `C01-rising-points` 展示“DP 严格递增条件解释错误但代码正确”的样本。
...
- [x] 完成阶段 2 最小纵向切片：A01/A02/B01/B02/C01 题目、参考解、测试、fixture、规则评估和 CLI demo
