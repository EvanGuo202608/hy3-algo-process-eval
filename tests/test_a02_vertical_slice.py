...
class A02VerticalSliceTests(unittest.TestCase):
    def _load(self, name):
        return json.loads((ROOT / "fixtures/a02_interval_removal" / name).read_text())

    def test_reference_solution_passes_tests(self):
        code = (ROOT / "benchmarks/problems/A02-interval-removal/reference_solution.cpp").read_text()
        cases = collect_test_cases(ROOT / "benchmarks/problems/A02-interval-removal")
    ...
    def test_overlap_bug_fails_tests(self):
        solution = self._load("wrong_code_no_overlap.json")
        cases = collect_test_cases(ROOT / "benchmarks/problems/A02-interval-removal")
        program = compile_and_run_cpp(solution["code"], cases)
