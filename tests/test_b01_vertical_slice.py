...
class B01VerticalSliceTests(unittest.TestCase):
    def _load(self, name):
        return json.loads((ROOT / "fixtures/b01_pairing" / name).read_text())

    def test_reference_solution_passes_tests(self):
        code = (ROOT / "benchmarks/problems/B01-pairing/reference_solution.cpp").read_text()
        cases = collect_test_cases(ROOT / "benchmarks/problems/B01-pairing")
    ...
    def test_lightest_first_bug_fails_tests(self):
        solution = self._load("wrong_code_lightest_first.json")
        cases = collect_test_cases(ROOT / "benchmarks/problems/B01-pairing")
        program = compile_and_run_cpp(solution["code"], cases)
