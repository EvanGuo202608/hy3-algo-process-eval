...
class D01VerticalSliceTests(unittest.TestCase):
    def _load(self, name):
        return json.loads((ROOT / "fixtures/d01_range_add_sum" / name).read_text())

    def test_reference_solution_passes_tests(self):
        code = (ROOT / "benchmarks/problems/D01-range-add-sum/reference_solution.cpp").read_text()
        cases = collect_test_cases(ROOT / "benchmarks/problems/D01-range-add-sum")
    ...
    def test_missing_length_multiplier_bug_fails_tests(self):
        solution = self._load("wrong_code_no_length.json")
        cases = collect_test_cases(ROOT / "benchmarks/problems/D01-range-add-sum")
        program = compile_and_run_cpp(solution["code"], cases)
