...
class C01VerticalSliceTests(unittest.TestCase):
    def _load(self, name):
        return json.loads((ROOT / "fixtures/c01_rising_points" / name).read_text())

    def test_reference_solution_passes_tests(self):
        code = (ROOT / "benchmarks/problems/C01-rising-points/reference_solution.cpp").read_text()
        cases = collect_test_cases(ROOT / "benchmarks/problems/C01-rising-points")
    ...
    def test_nondecreasing_transition_bug_fails_tests(self):
        solution = self._load("wrong_code_nondecreasing.json")
        cases = collect_test_cases(ROOT / "benchmarks/problems/C01-rising-points")
        program = compile_and_run_cpp(solution["code"], cases)
